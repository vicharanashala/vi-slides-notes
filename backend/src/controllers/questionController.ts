...
export const createQuestion = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (session.status === 'inactive') {
            res.status(403).json({ success: false, message: 'Session has not started yet. Questions cannot be submitted at this time.' });
            return;
        }
        ...
        if (session.status === 'ended') {
            res.status(403).json({ success: false, message: 'Session has ended. No new questions can be submitted.' });
            return;
        }
