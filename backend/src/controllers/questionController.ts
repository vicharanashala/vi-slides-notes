...
export const createQuestion = async (
  ...
  try {
    ...
    if (session.status === "paused") {
      res.status(403).json({
        success: false,
        message: "Session is paused. Please wait for teacher to resume.",
      });
      return;
    }
    ...
    if (session.status === "ended") {
      res.status(403).json({
        success: false,
        message: "Session has ended. No more questions can be submitted.",
      });
      return;
    }
...
export const deleteQuestion = async (
  ...
  try {
    ...
    const session = await Session.findById(question.session).populate(
      "teacher"
    );

    // Check if user is owner OR teacher of the session
    // For guest questions (no user), only teacher can delete
    const isOwner = question.user
      ? question.user.toString() === req.user?._id.toString()
      : false;
    const isTeacher = session?.teacher.toString() === req.user?._id.toString();
...
export const togglePin = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (session.teacher.toString() !== req.user?._id.toString()) {
      res.status(403).json({
        success: false,
        message: "Only the teacher can pin questions",
      });
      return;
    }
...
export const respondToQuestion = async (
  ...
  try {
    ...
    if (session.teacher.toString() !== req.user?._id.toString()) {
      res.status(403).json({
        success: false,
        message: "Only the teacher can respond to questions",
      });
      return;
    }
...
export const toggleUpvote = async (
  ...
  try {
    ...
    const isUpvoted = question.upvotes
      .map((id) => id.toString())
    ...
    res
      .status(500)
      .json({ success: false, message: "Server error toggling upvote" });
  }
};
...
export const requestAIAnalysis = async (
  ...
  try {
    ...
    if (session.teacher.toString() !== req.user?._id.toString()) {
      res.status(403).json({
        success: false,
        message: "Only the teacher can request AI analysis",
      });
      return;
    }
    ...
    (async () => {
      try {
        ...
        const analyzedQuestion = await Question.findById(question._id).populate(
          "user",
          "name"
        );
        emitToSession(session.code, "question_analyzed", analyzedQuestion);
...
export const processBatch = async (
  ...
  try {
    ...
    if (session.teacher.toString() !== req.user?._id.toString()) {
      res.status(403).json({
        success: false,
        message: "Only the teacher can trigger batch processing",
      });
      return;
    }

    // Import here to avoid circular dependency
    const { triggerBatchProcessing } = await import(
      "../services/questionBatchService"
    );
    ...
    res.status(202).json({
      success: true,
      message:
        "Batch refinement triggered. Questions will be refined and updated shortly.",
    });
  } catch (error) {
