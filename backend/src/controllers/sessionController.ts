...
const getLocalUrl = (): string => {
  ...
  if (detectedIp) {
    return `http://${detectedIp}:${frontendPort}`;
  }

  // 4. Ultimate Fallback: Default Localhost
  return envUrl || `http://localhost:${frontendPort}`;
};
...
// 👇 FORCE async execution AFTER response
setImmediate(async () => {
    ...
    for (const student of students) {
      try {
        ...
        if (result.success) {
          ...
          console.error(`❌ Email failed for ${student.email}:`, result.message, result.error);
        }
      } catch (err) {
        console.error(`❌ Exception while sending to ${student.email}`, err);
      }
    }
  } catch (err) {
...
// @desc    Send test session invitation emails to ALL students (real session link)
// @route   POST /api/sessions/test-email
// @access  Private (Teacher only)
export const sendTestSessionInvitation = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    // Use a currently active session to send actual session URL
    ...
    const sessionTitle = session.title || "Active session invitation";
    const sessionCode = session.code;
    // const joinUrl = session.joinUrl || `${getLocalUrl()}/session/${sessionCode}`;
    const joinUrl = `${process.env.FRONTEND_URL}/session/${sessionCode}`;
    const results = [];
    for (const student of students) {
      try {
        const result = await sendSessionInvitation(
        ...
        results.push({ email: student.email, success: false, message: "Exception during send" });
      }
    }
    ...
    res.status(500).json({ success: false, message: "Test invite failed", error });
  }
};
...
// @access  Private (Teacher only)
export const getOrCreateQuerySession = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
...
// @access  Private (Teacher only)
export const updateQueryUrl = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    if (!session) {
      res
        .status(404)
        .json({ success: false, message: "Query session not found" });
      return;
    }
