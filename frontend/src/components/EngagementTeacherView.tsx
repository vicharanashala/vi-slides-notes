...
const EngagementTeacherView: React.FC = () => {
  const [understandingMap, setUnderstandingMap] = useState<
    Map<string, UnderstandingUpdate>
  >(new Map());
  const [handRaisedMap, setHandRaisedMap] = useState<
    Map<string, HandRaiseUpdate>
  >(new Map());
  ...
  const stats = {
    confused: Array.from(understandingMap.values()).filter(
      (u) => u.understanding === "confused",
    ).length,
    neutral: Array.from(understandingMap.values()).filter(
      (u) => u.understanding === "neutral",
    ).length,
    understanding: Array.from(understandingMap.values()).filter(
      (u) => u.understanding === "understanding",
    ).length,
  };
    ...
    >
        ...
        >
          {total > 0 ? (
            <>
              <div
                style={{
                  width: `${(stats.confused / total) * 100}%`,
                  background: "#ef4444",
              ...
              <div
                style={{
                  width: `${(stats.understanding / total) * 100}%`,
                  background: "#10b981",
                  transition: "width 0.5s ease",
                }}
                title="Understanding"
      ...
      <div>
        ...
        >
          ...
          <span
            style={{
              color: handRaisedMap.size > 0 ? "var(--color-warning)" : "",
            }}
          >
            {handRaisedMap.size}
          </span>
