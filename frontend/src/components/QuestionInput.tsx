...
const QuestionInput: React.FC<QuestionInputProps> = ({ sessionId, sessionStatus, onQuestionSubmitted }) => {
    const isDisabled = sessionStatus === 'paused' || sessionStatus === 'ended' || sessionStatus === 'inactive';
            ...
            <form onSubmit={handleSubmit}>
                <div className="form-group" style={{ marginBottom: '0.75rem', position: 'relative' }}>
                    <textarea
                        ...
                        placeholder={isDisabled ? (sessionStatus === 'ended' ? "Session has ended..." : sessionStatus === 'inactive' ? "Session not started yet..." : "Session is paused by teacher...") : "Ask a question..."}
                        rows={3}
                        style={{
                            resize: 'none',
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    ...
                    >
                        {loading ? <div className="spinner" style={{ width: '16px', height: '16px', borderWidth: '2px' }}></div> : (isDisabled ? (sessionStatus === 'ended' ? 'Session Ended' : sessionStatus === 'inactive' ? 'Not Started' : 'Paused') : 'Send Question')}
                    </button>
                </div>
            </form>
        </div>
