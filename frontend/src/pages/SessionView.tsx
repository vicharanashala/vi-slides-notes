...
const SessionView: React.FC = () => {
  ...
  const [showWhiteboard, setShowWhiteboard] = useState(false);
  const [activePrivateChatRecipient, setActivePrivateChatRecipient] = useState<{
    id: string;
    name: string;
  } | null>(null);
  const [pulseCheckActive, setPulseCheckActive] = useState(false);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (sessionRes.success) {
          ...
          // 2. Fetch Questions
          const questionsRes = await questionService.getSessionQuestions(
            sessionRes.data._id
          );
          if (questionsRes.success) {
            setQuestions(
          ...
          socketService.onSessionStatusUpdate((data) => {
            ...
            if (data.status === "paused") {
              setToast({
                message: "⏸️ Teacher has paused the session",
                type: "warning",
              });
            }
            if (data.status === "active") {
              setToast({
                message: "▶️ Session resumed by teacher",
                type: "success",
              });
            }
            if (data.status === "ended") {
          ...
          socketService.onQuestionAnalyzed((analyzedQ: Question) => {
            setQuestions((prev) =>
              prev.map((q) => (q._id === analyzedQ._id ? analyzedQ : q))
            );
          });
          ...
          socketService.onQuestionsRefined((data: any) => {
            ...
            setToast({
              message: `Refined ${data.count} questions for clarity`,
              type: "info",
            });
          });
          ...
          socketService.onBatchRefinementFailed((data: any) => {
            ...
            setToast({
              message: "AI Refinement failed, retrying...",
              type: "warning",
            });
          });
          ...
          socketService.onUserJoined((newUser: any) => {
            ...
            setSession((prev) => {
              if (!prev) return null;
              // Check if student already exists
              const exists = prev.students.find(
                (s: any) => s._id === newUser._id || s.id === newUser._id
              );
              if (exists) return prev;
          ...
          socketService.onPulseCheckStart(() => {
            if (user?.role?.toLowerCase() === "student") {
              ...
              setPulseCheckTimer(10);
              setToast({
                message: "🔥 PULSE CHECK! Click the button for bonus points!",
                type: "warning",
              });
            }
          });
          ...
          socketService.onPointsUpdated((data) => {
            ...
            if (data.userId === user?.id) {
              setToast({
                message: `🎉 You got points! Total: ${data.points}`,
                type: "success",
              });
            }
          });
      ...
      try {
        ...
        if (questionsRes.success) {
          setQuestions((prev) => {
            const newQuestions = questionsRes.data;
            // Check for new teacher answers
            const hasNewTeacherAnswers = newQuestions.some(
              (nq) =>
                nq.teacherAnswer &&
                !prev.find(
                  (pq) =>
                    pq._id === nq._id && pq.teacherAnswer === nq.teacherAnswer
                )
            );
  ...
  const triggerPulseCheck = () => {
    if (isTeacher && code) {
      socketService.emitPulseCheckInit(code);
      setToast({
        message: "Pulse check triggered for students!",
        type: "info",
      });
    }
  };
  ...
  const handlePauseSession = async () => {
    ...
    try {
      ...
      if (response.success) {
        setSession((prev) =>
          prev ? { ...prev, status: response.status as any } : null
        );
      }
    } catch (err) {
      console.error("Error toggling pause:", err);
  ...
  const handleExportPDF = () => {
    ...
    const html = `
            ...
            <body>
                <div class="header">
                    <h1>${session?.title}</h1>
                    <p>Session Code: <strong>${
                      session?.code
                    }</strong> | Date: ${date} | Total Questions: ${
      questions.length
    }</p>
                </div>
                <div class="questions-list">
  ...
  const confirmEndSession = async () => {
    ...
    try {
      const response = await sessionService.endSession(session._id);
      if (response.success) {
        navigate(`/session/${session.code}/summary`, {
  ...
  const handleNextQuestion = () => {
    ...
    if (currentIndex === -1 || currentIndex === sortedQuestions.length - 1) {
      if (currentIndex === -1 && sortedQuestions.length > 0)
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--color-bg)" }}>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Question{" "}
              {sortedQuestions.findIndex((q) => q._id === selectedQuestionId) +
                1}{" "}
              of {questions.length}
            </div>
          ...
          <div style={{ flex: 1, overflowY: "auto", paddingRight: "0.5rem" }}>
            ...
            <Leaderboard
              students={session.students.filter(
                (student) => student._id !== session.teacher?._id
              )}
            />
      ...
      {showEndModal && (
        ...
        >
          ...
          >
            ...
            >
              This will stop all submissions and generate a final report. This
      ...
      {pulseCheckActive && user?.role?.toLowerCase() === "student" && (
        ...
        >
          <div
            className="glass-card anim-scale-up"
            style={{
              width: "300px",
              padding: "2rem",
              textAlign: "center",
          ...
          >
            <div style={{ fontSize: "3rem", marginBottom: "1rem" }}>⚡</div>
            <h2
              style={{ marginBottom: "0.5rem", color: "var(--color-warning)" }}
            >
              WAKE UP!
            </h2>
            <p
      ...
      {showConfetti && (
        <Confetti duration={5000} onComplete={() => setShowConfetti(false)} />
      )}
      {/* QR Code Modal for Session View */}
      {showQRModal && session && (
        <div
