...
const GuestJoinForm: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSessionInfo = async () => {
      ...
      try {
        ...
        // If guest hits email link and is not logged in, force login first
        if (!user) {
          navigate("/login", {
            replace: true,
        ...
        // If logged in, auto-join session and redirect
        if (user) {
          const joinRes = await sessionService.joinSession(code);
          if (joinRes.success) {
        ...
        if (err.message === "Network Error") {
          setError(
            "Connection Error: Cannot reach server. Please check your WiFi connection.",
          );
  ...
  const handleUserJoin = async () => {
    ...
    try {
      const response = await sessionService.joinSession(code);
      if (response.success) {
        navigate(`/session/${code}`);
        return;
      }
      setJoinError("Unable to join session. Please try again.");
    } catch (err: any) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {success && (
          <div
            className="alert alert-success slide-in"
            style={{
              marginBottom: "1rem",
              padding: "0.5rem",
              fontSize: "0.85rem",
        ...
        {user ? (
          ...
          >
            First, sign in with Google to join the live session directly.{" "}
